...
@Component({
  selector: 'app-display-queue-finance',
  templateUrl: './display-queue-finance.component.html',
  styles: [
    `
    .thai-sarabun {
...
export class DisplayQueueFinanceComponent implements OnInit, OnDestroy {
  ...
  async getRomService() {
    try{
       ...
       console.log(this.servicePointName);
       this.getList(this.servicePointId);

    //   showRooms(this.servicePointId);
  ...
  async getWaiting() {
    try {
      ...
      const rsTotal: any = await this.queueService.getWaitingnolimit(this.servicePointId, this.offset, sort, this.query);

      if (rs.statusCode === 200) {
        this.waitingItems = rs.results;
        this.total = rs.total;
        for (let index = 0; index < rsTotal.results.length; index++) {
          if(rsTotal.results[index].priority_id == '2'){
             this.totalOld = this.totalOld + 1;
          }else{
             this.totalNew = this.totalNew +1;
          }        
         }
        //  console.log(this.total);
        //  console.log(this.totalOld);
  ...
  getQueueFromRoom(room_id) {
    if (this.workingItems.length > 0) {
      const result = this.workingItems.find(e => e.room_id === room_id)
      if (result) {
